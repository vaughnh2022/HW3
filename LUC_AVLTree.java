...
public class LUC_AVLTree {
     ...
     *  in the tree:
     *    1. leaf node - simpliest case, just return null (which removes node) x
     *    2. interior node with only left subtree below it (node gets replaced x
     *       with left subtree)
     *    3. interior node with only right subtree below it (node gets replaced x
     *       with right subtree)
     ...
     */
    // catcher
    private Node findElementToDelete(Node cur,int val){
        if(cur==null){ // base null case
            return null;
        }
        if(val==cur.value){ // checks all possible cases if val == cur.value
            if(cur.leftChild==null&&cur.rightChild==null){ //returns null if both children are null
                return null;
            } else if(cur.leftChild==null&&cur.rightChild!=null){ //returns rightchild if leftchild is null
                return cur.rightChild;
            } else if(cur.leftChild!=null&&cur.rightChild==null){ //return leftchild if rightchild is null
                return cur.leftChild;
            } else if(cur.leftChild!=null&&cur.rightChild!=null) { // honestly still confused by this
                Node replaceCur = minValueNode(cur.rightChild);
                cur.value=replaceCur.value;
                cur.rightChild=findElementToDelete(cur.rightChild, replaceCur.value);
            }
        } else if(val<cur.value){ //calls for the leftchild to be able to effect the actual tree
            cur.leftChild=findElementToDelete(cur.leftChild, val);
        } else if(val>cur.value){ //calls for the rightChild to be able to effect the actual tree
            cur.rightChild=findElementToDelete(cur.rightChild, val);
        }
        cur.height=Math.max(getHeight(cur.leftChild),getHeight(cur.rightChild))+1;//updates height
        return autoBalancer(cur);//returns cur
    }
    private Node deleteElement(int value, Node node) {
        return findElementToDelete(node, value);
    }
    private Node autoBalancer(Node cur){
        if(cur==null){
            return null;
        }
        int balanceInt = getBalanceFactor(cur);//gets balance factor for cur
        if(balanceInt>1){ //if left heavy
            if(getBalanceFactor(cur.leftChild)<0){//looks if we want to LR rotate
                return LRRotation(cur);
            } 
              return LLRotation(cur); //RR rotate
        } else if(balanceInt<-1){//if right heavy
            if(getBalanceFactor(cur.rightChild)>0){//looks if we want to do RL rotation
                return RLRotation(cur); 
            }
            return RRRotation(cur); //LL rotate
        }
        return cur;//non rotation
    }
    /**
     *  Method getBalance
     *
